...
import com.google.collide.plugin.client.gwt.GwtCompilePlace.NavigationEvent;
...
implements GwtCompilerService
{
  ...
  public GwtCompileNavigationHandler(AppContext context, MultiPanel<?,?> masterPanel, Place currentPlace) {
    ...
    //load defaults based on user preferences
    this.model.setOpenSelf(true);
  ...
  protected void enterPlace(GwtCompilePlace.NavigationEvent navigationEvent) {
    ...
    if (run) {
      ...
      model.setModule(navigationEvent.getModule());
      JsoArray<String> all = JsoArray.from(navigationEvent.getSourceDirectory());
      all.addAll(navigationEvent.getLibsDirectory());
      model.setClasspath(all);
